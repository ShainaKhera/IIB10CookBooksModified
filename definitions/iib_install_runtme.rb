...
define :iib_install_runtime  do
  ...
  execute "Run IBM Integration Bus installer" do
    ...
    returns [0,1,127]
    cwd "#{unpack_dir}"
    command "#{unpack_dir}/iib make registry global accept license silently"
  end
  #
  ...
  execute "Update users profile to include curent IIB environment (mqsiprofile)" do
    user "root"
    returns [0]
    command "sed -i '$a source #{unpack_dir}/server/bin/mqsiprofile' #{home}/.bash_profile"
  end
  ...
  #  user "root"
  #  returns [0]
  #  command "sed -i '$a source #{unpack_dir}/server/bin/mqsiprofile' #{home}/.bashrc"
  #end
end
